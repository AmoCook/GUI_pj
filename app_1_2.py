...
class Solution():
    ...
    def DTW(self, A, B):
        if (len(A) == 0 and len(B) == 0):
            ...
            return (self.d(self.Head(A), self.Head(B)) + self.min_1(self.DTW(A, self.Rest(B)),
                                                                    self.DTW(self.Rest(A), B),
                                                                    self.DTW(self.Rest(A), self.Rest(B))))
...
class ButtonFrame(wx.Frame, object):
    ...
    def __init__(self, object):
        ...
        self.choice_1 = wx.Choice(panel, -1, choices=self.scores, pos=(352, 95), size=(80, 30))

        self.scores_n_w = ['1', '2', '3', '4', '5']
        self.choice_n_w = wx.Choice(panel, -1, choices=self.scores_n_w, pos=(410, 135), size=(80, 30))

        # set label
        self.input_filename = wx.TextCtrl(panel, -1, pos=(520, 100), size=(120, 19))
        self.varible = wx.TextCtrl(panel, -1, pos=(540, 140), size=(100, 19))

        self.information = wx.StaticText(panel, -1, '属性:', pos=(480, 140), size=(80, 10), style=wx.ALIGN_CENTER)
        self.information = wx.StaticText(panel, -1, '井口数:', pos=(340, 140), size=(80, 10), style=wx.ALIGN_CENTER)
        self.information = wx.StaticText(panel, -1, '输入文件名:', pos=(435, 100), size=(80, 10), style=wx.ALIGN_CENTER)
        ...
        self.information_1 = wx.TextCtrl(panel, -1,
                                         "\n1. 请选择想使用的匹配算法!!!\n\n2. 通过点击introduce了解相应算法!!!\n\n3. 写入输入数据的文件!!!\n\n4. 成功写入数据后,点击start开始匹配!!!",
                                         pos=(50, 180), size=(250, 300), style=wx.TE_READONLY)
        ...
        try:
            ...
            if (bool(re.match('.*xlsx', input_filename))):
                ...
                for item in range(int(well_num)):
                    ...
                    table = file.sheets()[item]
                    ncols = table.ncols
                    for col in range(ncols):
                        col_v = table.col_values(col)[0]
                        print(str(item), col_v)
                        if (col_v == var):
                            self.var_data.append(table.col_values(col)[1:])
                            self.depth_data.append(table.col_values(1)[1:])
                            print('depth_data', str(item+1), self.depth_data)
    ...
    def OnClick_start(self, event):
        ...
        if (choice_str == 'DTW'):
            num_w = self.choice_n_w.GetStringSelection()
            opt = self.solution.DTW_puls(self.var_data[0], self.var_data[1])
            self.reslut = []
            for item1 in range(int(num_w)):
                for item2 in range(item1):
                    key = self.solution.DTW_puls(self.var_data[item1], self.var_data[item2])
                    self.reslut.append([item1, item2, key])
            print(self.reslut)
            self.reslut.sort(key=solution.lastone)
            print(self.reslut)
            ...
            #         num_w = whichone
            #
            plt.plot(self.var_data[self.reslut[0][0]], self.depth_data[self.reslut[0][0]], color='red', label='e_str', alpha=0.7, linestyle='-.')
            # plt.legend(str(self.reslut[0][0]))

            plt.plot(self.var_data[self.reslut[0][1]], self.depth_data[self.reslut[0][1]], color='blue', label='input_str', alpha=0.7, linestyle='--')
            plt.legend('2','1')
            plt.xlabel(self.varible.GetValue())
            plt.ylabel('Depth')
            plt.show()
            # 修改信息栏
            self.information_1.Remove(0, 200)
            ...
            prefile = '\n\n=============================================\n\n' + '    当前时间为' + str(
                now_time) + '\n\n' + '       本次匹配的属性为' + self.varible.GetValue() + ',共有' + str(
                num_w) + '口井匹配\n\n' + 'DTW匹配结果按从优到劣的顺序排序(格式: 井1 井2 相似度)\n\n'
            file.write(prefile)

            for item1 in range(len(self.reslut)):
                file.write('           ' + str(self.reslut[item1][0]) + '         ' + str(
                    self.reslut[item1][1]) + '         ' + str(self.reslut[item1][2]) + '\n')
            file.close()
        ...
        if (choice_str == 'LCSS'):
            ...
            for item1 in range(int(num_w)):
                for item2 in range(item1):
                    c, flag = solution.lcs(self.var_data[item1], self.var_data[item2])
                    solution.li = []
                    solution.printLcs(flag, self.var_data[item1], len(self.var_data[item1]), len(self.var_data[item2]))
                    key = len(solution.li)
                    self.reslut.append([item1, item2, key])
            ...
            prefile = '\n\n=============================================\n\n' + '    当前时间为' + str(
                now_time) + '\n\n' + '       本次匹配的属性为' + self.varible.GetValue() + ',共有' + str(
                num_w) + '口井匹配\n\n' + 'LCSS匹配结果按从优到劣的顺序排序(格式: 井1 井2 相似度)\n\n'
            file.write(prefile)
